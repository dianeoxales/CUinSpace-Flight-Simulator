...
void manager_clean(Manager *manager) {
    system_array_clean(&manager->system_array); // clean up system array
    resource_array_clean(&manager->resource_array); // clean up resource array
    event_queue_clean(&manager->event_queue);
    
    manager->simulation_running = 0;
    // Reset the system array, resource array, and event queue to NULL or empty states
    manager->system_array.systems = NULL;
    manager->system_array.size = 0;
    manager->system_array.capacity = 0;

    manager->resource_array.resources = NULL;
    manager->resource_array.size = 0;
    manager->resource_array.capacity = 0;

    manager->event_queue.head = NULL;
    manager->event_queue.size = 0;
}
...
void *manager_thread(void *arg) {
    ...
    // loop until simulation flag is set to zero
