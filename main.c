...
int main(void) {
   ...
   // create thread for manager
    ...
    pthread_create(&manager_tid, NULL, &manager_thread, (void*)&manager);

    // create a thread for each system
    pthread_t *system_tids = malloc(manager.system_array.size * sizeof(pthread_t));
    for (int i = 0; i < manager.system_array.size; ++i) {
        pthread_create(&system_tids[i], NULL, system_thread, (void*)manager.system_array.systems[i]);
    }
...
void load_data(Manager *manager) {
    ...
    resource_create(&fuel, "Fuel", 5000, 5000);
    resource_create(&oxygen, "Oxygen", 50, 50);
